...
def remove_overlap(seg_out, warped_cm):
    """
    Loại bỏ sự chồng lấn giữa các đối tượng trong ảnh bằng cách điều chỉnh giá trị của 'warped_cm'.

    Parameters:
    - seg_out (torch.Tensor): Đầu ra của mô hình segmenter, có thể là các dự đoán đối tượng trong ảnh.
                             Yêu cầu có 6 kênh (channels) với các thông tin về đối tượng cụ thể.
    - warped_cm (torch.Tensor): Ma trận 4D biểu diễn ảnh đã biến đổi (warped). Cần có ít nhất 4 chiều.

    Returns:
    - torch.Tensor: Ma trận 'warped_cm' đã được điều chỉnh để loại bỏ sự chồng lấn giữa các đối tượng.
    """

    # Đảm bảo 'warped_cm' có đúng số chiều
    assert len(warped_cm.shape) == 4

    # Tính toán giá trị điều chỉnh để loại bỏ sự chồng lấn
    adjustment = (torch.cat([seg_out[:, 1:3, :, :], seg_out[:, 5:, :, :]], dim=1)).sum(dim=1, keepdim=True)

    # Áp dụng điều chỉnh cho 'warped_cm'
    warped_cm = warped_cm - adjustment * warped_cm

    # Trả về ma trận 'warped_cm' đã được điều chỉnh
    return warped_cm
...
def get_opt():
    ...
    parser.add_argument('--name', type=str, required=True) # Tên đặc trưng cho mô hình hoặc quá trình huấn luyện (bắt buộc)
    #Các GPU sẽ được sử dụng, có thể là nhiều GPU, được phân tách bằng dấu phẩy
    parser.add_argument('--gpu_ids', type=str, default='0')
    # Số lượng worker sẽ được sử dụng để đọc dữ liệu đầu vào đồng thời
    parser.add_argument('-j', '--workers', type=int, default=4) 
    parser.add_argument('-b', '--batch_size', type=int, default=8)#  Kích thước batch
    parser.add_argument('--fp16', action='store_true', help='use amp') # Sử dụng mixed precision training để giảm bộ nhớ GPU
    # Cuda availability
    # Lựa chọn sử dụng GPU hoặc CPU
    parser.add_argument('--cuda',default=False, help='cuda or cpu')

    parser.add_argument("--dataroot", default="./data/") # Đường dẫn đến thư mục chứa dữ liệu
    parser.add_argument("--datamode", default="train") #Chế độ dữ liệu (train, test, ...)
    parser.add_argument("--data_list", default="train_pairs.txt") #Tên tệp chứa danh sách các cặp dữ liệu

    ## Kích thước ảnh sau khi tiền xử lý 
    parser.add_argument("--fine_width", type=int, default=768)
    parser.add_argument("--fine_height", type=int, default=1024)

    # Tham số liên quan đến việc tạo mặt nạ
    parser.add_argument("--radius", type=int, default=20)
    parser.add_argument("--grid_size", type=int, default=5)

    # Thư mục lưu thông tin TensorBoard và checkpoint
    parser.add_argument('--tensorboard_dir', type=str, default='tensorboard', help='save tensorboard infos')
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints', help='save checkpoint infos')

    # Đường dẫn đến checkpoint của các mô hình
    ...
    # Các thông số liên quan đến việc lưu thông tin và hiển thị trạng thái huấn luyện
    parser.add_argument("--tensorboard_count", type=int, default=100)
    parser.add_argument("--display_count", type=int, default=100)
    parser.add_argument("--save_count", type=int, default=10000)

    # Các thông số về quá trình huấn luyện
    parser.add_argument("--load_step", type=int, default=0)
    parser.add_argument("--keep_step", type=int, default=100000)
    parser.add_argument("--decay_step", type=int, default=100000)

    # Xáo trộn dữ liệu đầu vào
    parser.add_argument("--shuffle", action='store_true', help='shuffle input data')
    
    # test
    # Các thông số liên quan đến việc kiểm thử
